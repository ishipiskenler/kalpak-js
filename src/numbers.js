/**
 * Karakalpak Number to Text Converter
 * Converts numbers to Karakalpak words
 * Supports: integers, negative numbers, and decimals
 * Range: -1 trillion to 1 trillion
 */
...
/**
 * Converts a number to Karakalpak text
 * @param {number} num - The number to convert
 * @returns {string} - The number in Karakalpak words
 */
...
function numberToText(num) {
  ...
  if (num < -MAX || num > MAX) {
    return "Error: Number out of range (max: ±1 trillion)";
  }
  ...
  // Decimal numbers (pútin sanlar)
  if (!Number.isInteger(num)) {
    const [intPart, decPart] = num.toString().split(".");
    return `${numberToText(parseInt(intPart))} pútin ${numberToText(
      parseInt(decPart)
    )}`;
  }

  // Special cases for exact round numbers
